...
export default function Pizarra() {
  ...
  const abrirInterfazCompra = async (recompensa) => {
    ...
    try { 
      // Enviamos curso_id para filtrar alumnos elegibles correctamente
  ...
  const procesarCompra = async (alumnoId, nombreAlumno) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({ title: 'Error', text: 'No se pudo procesar la compra o puntos insuficientes.', icon: 'error', confirmButtonColor: '#EF4444' });
      }
    }
  };
    ...
    return (
      <div style={{ width: '100%', padding: '20px' }}>
        <header style={{ marginBottom: '30px', textAlign: 'center' }}>
          ...
          <p style={{ color: '#64748B', fontSize: '1.1rem' }}>Elige hasta 5 premios para mostrar, o usa el azar.</p>
        </header>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', backgroundColor: '#F8FAFC', padding: '25px', borderRadius: '20px', border: '1px solid #E2E8F0' }}>
          <button onClick={presentarAzar} style={btnMain('#F59E0B')}>🎲 Sorprender (Al Azar)</button>
          <button onClick={presentarSeleccion} style={{ ...btnMain('#6366F1'), opacity: seleccionados.length === 0 ? 0.5 : 1 }} disabled={seleccionados.length === 0}>📺 Presentar Seleccionados ({seleccionados.length}/5)</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', backgroundColor: '#F8FAFC', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'fixed', top: 0, left: 0, zIndex: 9999 }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '50px', animation: animando ? 'fadeInDown 0.8s ease-out' : 'none' }}>
        <h1 style={{ fontSize: '4rem', color: '#1E293B', fontWeight: '900', textShadow: '0 4px 6px rgba(0,0,0,0.05)' }}>✨ Premios del Día ✨</h1>
        <p style={{ fontSize: '1.5rem', color: '#64748B', margin: 0 }}>¡Haz clic en una tarjeta para canjear tus puntos!</p>
      </div>
      ...
      <div style={{ display: 'flex', gap: '25px', flexWrap: 'wrap', justifyContent: 'center', perspective: '1000px' }}>
        {premiosPizarra.map((premio, index) => (
          ...
          >
            <div style={{ backgroundColor: '#FCD34D', color: '#92400E', padding: '10px 25px', borderRadius: '25px', fontWeight: '900', fontSize: '1.4rem', marginBottom: '40px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>{premio.costo} PTS</div>
            <h2 style={{ color: 'white', fontSize: '1.8rem', textAlign: 'center', fontWeight: '900', lineHeight: '1.2' }}>{premio.nombre}</h2>
              ...
              <div style={{ display: 'grid', gap: '12px', maxHeight: '40vh', overflowY: 'auto', paddingRight: '5px' }}>
                {alumnosElegibles.map(alumno => (
                  <div key={alumno.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px 20px', border: '1px solid #E2E8F0', borderRadius: '12px', backgroundColor: '#F8FAFC' }}>
                    ...
                    <button onClick={() => procesarCompra(alumno.id, alumno.nombre)} style={{ padding: '10px 18px', backgroundColor: '#10B981', color: 'white', border: 'none', borderRadius: '10px', fontWeight: '800', cursor: 'pointer', boxShadow: '0 4px 6px rgba(16, 185, 129, 0.2)' }}>Canjear</button>
                  </div>
                ))}
              </div>
            )}
            <button onClick={() => setRecompensaActiva(null)} style={{ padding: '15px', width: '100%', marginTop: '25px', backgroundColor: '#F1F5F9', color: '#475569', border: 'none', borderRadius: '12px', fontWeight: '800', cursor: 'pointer' }}>Cerrar</button>
          </div>
        </div>
      )}
...
const btnMain = (bg) => ({ padding: '15px 30px', backgroundColor: bg, color: 'white', border: 'none', borderRadius: '15px', fontWeight: '900', fontSize: '1.1rem', cursor: 'pointer', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', transition: 'transform 0.1s' });
...
const modalStyle = { backgroundColor: '#fff', padding: '35px', borderRadius: '24px', width: '100%', maxWidth: '450px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)' };