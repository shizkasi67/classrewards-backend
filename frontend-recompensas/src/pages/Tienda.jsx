...
export default function Tienda() {
  ...
  const cargarDatos = async () => {
    try {
      ...
      if (listaCursos.length > 0 && !cursoActual) setCursoActual(listaCursos[0].id);
      setRecompensas(listaRec);
  ...
  const abrirModalCompra = async (recompensa) => {
    if (!cursoActual) return Swal.fire({ title: 'Atención', text: 'Selecciona un curso primero.', icon: 'info', confirmButtonColor: '#6366F1' });
  ...
  const procesarCompraMultiple = async () => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({ title: 'Error', text: 'Hubo un problema con la compra masiva.', icon: 'error', confirmButtonColor: '#EF4444' });
      }
    }
  };
  ...
  const guardarNuevaRecompensa = async () => {
    ...
    try {
      await db.crearRecompensa(nuevoNombreRec, Number(nuevoCostoRec), nuevaDescRec);
  ...
  const eliminarRecompensa = async (id, nombre, e) => {
    ...
    const result = await Swal.fire({
      ...
      html: `Estás a punto de borrar <b>"${nombre}"</b>.`,
    ...
    if (result.isConfirmed) {
      try {
        await db.eliminarRecompensa(id);
        cargarDatos();
        Swal.fire({ title: 'Eliminado', icon: 'success', timer: 1000, showConfirmButton: false });
      } catch { Swal.fire('Error', 'No se pudo eliminar el premio.', 'error'); }
    }
  };
    ...
    <div style={{ width: '100%' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '40px', backgroundColor: '#6366F1', padding: '30px', borderRadius: '20px', color: 'white', boxShadow: '0 10px 15px -3px rgba(99, 102, 241, 0.3)', flexWrap: 'wrap', gap: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '15px', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <button onClick={() => setModalNuevaRecompensa(true)} style={{ backgroundColor: '#F59E0B', color: 'white', border: 'none', padding: '12px 20px', borderRadius: '15px', fontWeight: '800', cursor: 'pointer' }}>
            + Nuevo Premio
          </button>
        </div>
      </header>
        ...
        <div style={overlayStyle}>
          <div style={modalStyle}>
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '10px' }}>
              ...
              <button type="button" onClick={guardarNuevaRecompensa} style={{ padding: '12px 25px', backgroundColor: '#F59E0B', color: 'white', border: 'none', borderRadius: '12px', fontWeight: '800', cursor: 'pointer' }}>Guardar Premio</button>
            </div>
          </div>
        </div>
      )}
            ...
            {cargandoElegibles ? <p style={{ textAlign: 'center', padding: '20px', color: '#64748B' }}>Buscando alumnos...</p> : alumnosElegibles.length === 0 ? (
              ...
              <>
                  ...
                  {alumnosElegibles.map(alumno => {
                    ...
                    return (
                      <div key={alumno.id} onClick={() => toggleAlumnoSeleccionado(alumno.id)}
                        style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px 20px', border: isSelected ? '2px solid #6366F1' : '1px solid #E2E8F0', borderRadius: '12px', backgroundColor: isSelected ? '#EEF2FF' : '#F8FAFC', cursor: 'pointer', transition: 'all 0.2s' }}>
                        <div>
                        ...
                        <div style={{ width: '24px', height: '24px', borderRadius: '50%', border: '2px solid #6366F1', backgroundColor: isSelected ? '#6366F1' : 'transparent', display: 'flex', justifyContent: 'center', alignItems: 'center', color: 'white', fontSize: '0.8rem' }}>
                          {isSelected && '✓'}
                        </div>
                      </div>
                    );
                ...
                <button onClick={procesarCompraMultiple} disabled={alumnosSeleccionados.length === 0}
                  style={{ padding: '15px', width: '100%', marginTop: '20px', backgroundColor: alumnosSeleccionados.length > 0 ? '#10B981' : '#CBD5E1', color: 'white', border: 'none', borderRadius: '12px', fontWeight: '800', cursor: alumnosSeleccionados.length > 0 ? 'pointer' : 'default' }}>
